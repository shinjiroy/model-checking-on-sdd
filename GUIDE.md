...
モデル検査は、システムの状態空間を網羅的に探索して、システム設計が特定のプロパティを満たすことを検証します。コード記述前に設計が健全であることを検証します。

### なぜAlloy?

- **Webアプリケーション向き**: ユーザー、商品、注文、関係性のモデリングが自然
- **視覚的に確認も可能**: 作られたファイルをGUIツールに読み込ませて反例をグラフとして表示
- **適度な学習曲線**: 一階述語論理 + 関係論理
...
### 1. 1つの論理的機能 = 1つのAlloyモデル

**ガイドライン**: 論理的に独立した機能ごとに1つの`.als`ファイルを作成します（推奨、強制ではない）。
...
└── formal/
    ├── purchase.als          # 購入フロー
    ├── inventory.als         # 在庫管理（独立した論理機能）
    └── payment.als           # 決済処理（独立した論理機能）
```

**理由は?**

- 明確なトレーサビリティを維持(plan.md ↔ model.als)
- レビューとメンテナンスが簡略化

**複数モデルが必要な場合:**

1. 各モデルが独立した論理機能を表しているか確認
2. 独立していれば複数モデルでOK
3. 相互依存が強い場合は、仕様自体の分割も検討（ただし強制ではない）
...
1. **assertionを特定**: `.als`ファイルで`assert PropertyName { ... }`を探し、何を検証しているか理解
2. **counterexampleを解釈**: Skolem変数（例: `$PropertyName_o = Order$0`）がどのインスタンスが違反しているかを示す。Instance Dataがあれば直接使用、なければSkolem変数 + モデル制約から推測
3. **根本原因を特定**: なぜ失敗するか（制約不足、設計の隙間など）を特定し、修正戦略を決定

**修正を決定**:
- **制約不足?** → `.als`ファイルのpredicateまたはfactに制約を追加
- **設計の隙間?** → `plan.md`を更新して要件を明確化
- **有効なエッジケース?** → 既知の制限として`properties.md`に文書化
...
### 5. 無関係な機能を1つのモデルに詰め込む

**問題**: 論理的に独立した機能を1つの巨大なモデルに詰め込む。

```text
❌ specs/001-feature/formal/
    └── everything.als    # 500行以上、複数の独立機能が混在
```

**解決策**: 論理的に独立した機能ごとにモデルを分割（推奨）。または、仕様自体を分割することも検討。
