...
LLMエージェントを利用した開発手法として仕様駆動開発が主流となりつつあります。ここでは[Spec Kit](https://github.com/github/spec-kit)を使うことを想定します。

Spec Kitではドキュメントを検証する機構がありますが、仕様が曖昧でないことや複数のドキュメント間の整合性のチェック等を自然言語的に検証するのみです。  
仕様、実装計画に書いてある事の正しさの確認は、人間やLLMエージェントによる自然言語ベースのレビューで行うこととなります。特に、特定のデータに対して色々なパターンや並列で読み書きが発生する場合の実装計画をレビューするのは難しいです。  
ここでは実装計画(specify.plan)で作成されたドキュメントをモデル検査で検証することを検討します。

また、一般的な開発フローに組み込むことを想定するため、形式手法やモデル検査のプロフェッショナルでなくとも恩恵が受けられるようにします。  
形式手法やモデル検査のためのツールはいくつかありますが、比較的簡単に使えるツールのAlloyを採用してみます。
